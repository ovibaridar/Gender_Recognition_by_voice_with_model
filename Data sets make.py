...
data_frame = pd.DataFrame(
    columns=['mean_freq', 'sd_freq', 'median_freq', 'q25_freq', 'q75_freq', 'iqr_freq', 'skewness', 'kurtosis_val',
             ...
             'max_fun', 'mean_dom', 'min_dom', 'max_dom', 'fund_freq', 'mod_index', 'label'])
...
for i in range(0, 2312):
    ...
    if i == 163:
        ...
        audio_file = f'audio file/Female/{i}.wav'
    ...
    # Create a DataFrame to store the results
        data_frame.loc[i] = [mean_freq, sd_freq, median_freq, q25_freq, q75_freq, iqr_freq, skewness, kurtosis_val,
                         spectral_flatness, tempogram_mean, mode_freq, centroid_freq, peak_freq, mean_fun, min_fun,
                         max_fun, mean_dom, min_dom, max_dom, fund_freq.mean(), mod_index, label]
        print(audio_file)

# data_frame.to_csv('voice2.csv', index=False)
